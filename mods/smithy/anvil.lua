...
local smithy_anvil_formspec =
	"size[8,8]"..
	"image[4.25,3;1,1;glooptest_tool_steelhammer.png]"..
	"list[current_name;input;2.5,0;3,3;]"..
	"list[current_name;hammer;5.25,3;1,1;]"..
	"label[1,1.25;"..SL("Workspace:").."]"..
	"label[6.25,3;"..SL("Optional").."]"..
	"label[6.25,3.3;"..SL("storage for").."]"..
	"label[6.25,3.6;"..SL("your hammer").."]"..
	"label[0,0;"..SL("Anvil").."]"..
	"label[0,3;"..SL("Punch anvil with hammer to repair").."]"..
	"label[0,3.3;"..SL("tool or forge metal in workpiece-slot.").."]"..
...
minetest.register_node(":castle:anvil", {
	...
	allow_metadata_inventory_put = function(pos, listname, index, stack, player)
		...
		-- 	SL('The workpiece slot is for damaged tools only.'));
		-- 	return 0;
		-- end
	...
	on_punch = function(pos, node, puncher)
		...
		if( not( wielded ) or not( wielded:get_name() ) or wielded:get_name() ~= 'smithy:hammer') then
			return;
		end
