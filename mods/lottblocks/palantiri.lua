		...
		elseif meta:get_int("configured") == 1 then
			...
			if can_tp == true then
				...
				if fields.teleports == SL("Teleport to...") then
					if minetest.registered_nodes[minetest.get_node({x = pos.x, y = pos.y + 2, z = pos.z}).name].walkable or
						 minetest.registered_nodes[minetest.get_node({x = pos.x, y = pos.y + 1, z = pos.z}).name].walkable then
							 minetest.chat_send_player(sender:get_player_name(), minetest.colorize(purple,SL("Sorry, at the point of teleport wall")))
							return
					end
					--print(player_name)
