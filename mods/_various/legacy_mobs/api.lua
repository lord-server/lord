...
if rawget(_G, "invisibility") then
	legacy_mobs.invis = invisibility
end
...
local function follow_holding(self, clicker)
	...
	if legacy_mobs.invis[clicker:get_player_name()] then
		return false
	end
...
local mob_attack = function(self)
	...
	for n = 1, #objs do
		...
		if objs[n]:is_player() then
			...
			if legacy_mobs.invis[ player_name ] then
				type = ""
			else
...
local follow_flop = function(self)
	...
	and self.state ~= "runaway" then
		...
		for n = 1, #players do
			...
			and not legacy_mobs.invis[ players[n]:get_player_name() ] then
...
local do_states = function(self, dtime)
	...
	elseif self.state == "attack" then
		...
		or (self.attack:is_player() and legacy_mobs.invis[ self.attack:get_player_name() ]) then
			...
			if dist > self.reach then
				...
				if self.timer > 3 then
					...
					or minetest.is_protected(pos, "") then
						...
						-- hurt player/legacy_mobs caught in blast area
					...
					legacy_mobs:explosion(pos, radius, 1, 1, self.sounds.explode)
...
function legacy_mobs.mob_punch(self, hitter, tflp, tool_capabilities, dir)
	...
	-- attack puncher and call other legacy_mobs for help
	if self.passive == false
	and self.state ~= "flop"
	and self.child == false
	and (hitter.is_player == nil or hitter:is_player() == false or (hitter:get_player_name() ~= self.owner
	and not legacy_mobs.invis[ hitter:get_player_name() ])) then
...
local mob_staticdata = function(self)
	...
	-- used to rotate older legacy_mobs
	if self.drawtype
...
local mob_activate = function(self, staticdata, def)
	...
	if not self.base_texture then
		...
		-- compatiblity with old simple legacy_mobs textures
...
-- default function when legacy_mobs are blown up with TNT
...
-- register mob entity
function legacy_mobs:register_mob(name, def)

	legacy_mobs.spawning_mobs[name:replace('^:', '')] = true
...
end -- END legacy_mobs:register_mob function


-- count how many legacy_mobs of one type are inside an area
...
function legacy_mobs:spawn_specific(name, nodes, neighbors, min_light, max_light,
	...
	minetest.register_abm({
		...
		action = function(pos, node, active_object_count, active_object_count_wider)
			...
			if not legacy_mobs.spawning_mobs[name] then
...
--print ("--- player too close", name)
			...
			-- legacy_mobs cannot spawn in protected areas when enabled
...
--				print ("[legacy_mobs] Spawned " .. name .. " at "
...
--				.. node.name .. " near " .. neighbors[1])
				...
				print (S("[legacy_mobs] @1 failed to spawn at @2",
...
function legacy_mobs:register_spawn(name, nodes, max_light, min_light, chance, active_object_count, max_height, day_toggle)
	...
	legacy_mobs:spawn_specific(name, nodes, {"air"}, min_light, max_light, 30,
		chance, active_object_count, -31000, max_height, day_toggle)
end


-- MarkBu's spawn function
function legacy_mobs:spawn(def)
	...
	legacy_mobs:spawn_specific(name, nodes, neighbors, min_light, max_light, interval,
...
function legacy_mobs:explosion(pos, radius, fire, smoke, sound)
...
-- register spawn eggs
function legacy_mobs:register_egg(mob, desc, background, addegg, no_creative)
...
function legacy_mobs:capture_mob(self, clicker, chance_hand, chance_net, chance_lasso, force_take, replacewith)
...
function legacy_mobs:protect(self, clicker)
...
function legacy_mobs:feed_tame(self, clicker, feed_count, breed, tame)
	...
	and clicker:get_player_name() == self.owner then
		...
		minetest.show_formspec(name, "legacy_mobs_nametag", "size[8,4]"
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	...
	if formname == "legacy_mobs_nametag"
	and fields.name
...
function legacy_mobs:alias_mob(old_name, new_name)
