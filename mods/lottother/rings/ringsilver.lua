...
-- Нам нужно продублировать minetest.item_drop чтобы в функция возвратила сущность...
		...
		minetest.after(3, function()
			if ent ~= nil and metadata ~= nil then
				...
				if pos ~= nil then
					...
					ent:remove()
					if node == "default:water_source" --[[(Из-за этого крашится игра) and os.time() - tonumber(metadata) <= 13]] then
...
for _, status in pairs({"active", "inactive"}) do
	...
	minetest.register_node("lottother:ringsilver_furnace_" .. status, {
		description = SL("Ringsilver Furnace"),
		tiles = {
...
minetest.register_craft({
	...
	recipe = {
		...
		{"default:obsidian", "lottblocks:dual_furnace_inactive", "default:obsidian"},
		{"default:obsidian", "default:obsidian", "default:obsidian"},
	},
})
