...
local function replacer_get_node(itemstack, pointed_thing, player_name)
	local pointed_pos = core.get_pointed_thing_position(pointed_thing)
	local selected_node = core.get_node_or_nil(pointed_pos)
	...
	if table_has_value(REPLACER_PLACE_BLACKLIST, selected_node.name) then
		core.chat_send_player(player_name, S("Error: this node is in blacklist."))
		return nil
	end
...
---@param itemstack ItemStack
---@return NodeTable @`core.get_node` format
...
local function replacer_set_node(itemstack, pointed_thing, player, place_above)
	...
	local pointed_pos = core.get_pointed_thing_position(pointed_thing, place_above)
...
core.register_tool("lord_replacer:replacer", {
