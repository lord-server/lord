
lottpotion = {}

dofile(minetest.get_modpath("lottpotion").."/potions/aka_api.lua")
dofile(minetest.get_modpath("lottpotion").."/benches/lottpotion_recipe_system.lua")


dofile(minetest.get_modpath("lottpotion").."/benches/helpers.lua")
dofile(minetest.get_modpath("lottpotion").."/benches/cauldron.lua")
dofile(minetest.get_modpath("lottpotion").."/benches/laboratory.lua")
dofile(minetest.get_modpath("lottpotion").."/benches/barrel.lua")


dofile(minetest.get_modpath("lottpotion").."/potions/arrows_aliases.lua")
dofile(minetest.get_modpath("lottpotion").."/potions/ingredients.lua")
dofile(minetest.get_modpath("lottpotion").."/potions/potions.lua")
dofile(minetest.get_modpath("lottpotion").."/potions/recipes.lua")
