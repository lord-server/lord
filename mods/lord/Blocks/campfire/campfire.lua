...
local function fire_particles_on(pos)
	...
	id = core.add_particlespawner({ -- 1 layer big particles fire
		amount             = 12,
		time               = 1.3,
	...
	id = core.add_particlespawner({ -- 2 layer smol particles fire
		amount             = 4,
		time               = 1.3,
	...
	id = core.add_particlespawner({ --3 layer smoke
		amount             = 10,
		time               = 1,
		...
		if string.find(core.serialize(eat_y), "do_item_eat") and meta:get_int("cooked_time") == 0 then
			...
			core.after(cooked.time / 2, function()
				if meta:get_int("it_val") > 0 then
					...
					core.after(cooked.time / 2, function(item) -- luacheck: ignore item
						if meta:get_int("it_val") > 0 then
							core.add_item({ x = pos.x, y = pos.y + 0.5, z = pos.z }, item)
							meta:set_int('cooked_time', 0);
							meta:set_int('cooked_cur_time', 0);
						else
							core.add_item({ x = pos.x, y = pos.y + 0.5, z = pos.z }, name)
						end
					end, item)
				else
					core.add_item({ x = pos.x, y = pos.y + 0.5, z = pos.z }, name)
...
-- NODES
core.register_node('campfire:fireplace', {
...
core.register_node('campfire:campfire', {
	...
	on_rightclick       = function(pos, node, player, itemstack, pointed_thing)
		if itemstack:get_name() == "default:torch" then
			...
			core.set_node(pos, { name = 'campfire:campfire_active' })
			core.add_particle({
...
core.register_node('campfire:campfire_active', {
...
core.register_abm({
	...
	action    = function(pos, node, active_object_count, active_object_count_wider)
		...
		if #fpos > 0 then
			core.set_node(pos, { name = 'campfire:campfire' })
			...
			if campfire_limit and campfire_ttl > 0 then
				if it_val <= 0 then
					...
					core.set_node(pos, { name = 'campfire:fireplace' })
					core.add_item(pos, "campfire:ash")
					return
				end
				meta:set_int('it_val', it_val);
			end
...
-- ITEMS
core.register_craftitem("campfire:ash", {
	description     = S("Ash"),
