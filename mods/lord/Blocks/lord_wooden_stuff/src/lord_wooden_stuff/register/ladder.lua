...
local function register_ladder(wood, def, _, stick)
	...
	core.register_node(name, {
		...
		on_place                  = function(itemstack, placer, pointed_thing)
			...
			local node_above_name = core.get_node(above).name
			if pointed_thing.type ~= "node" and	not core.registered_nodes[node_above_name].buildable_to then
				return
			end
			...
			if core.registered_nodes[node_under.name].on_rightclick then
				return core.registered_nodes[node_under.name].on_rightclick(under, node_under, placer, itemstack, pointed_thing)
			end
			...
			if param2 then
				core.set_node(above, { name = name, param2 = param2 })
