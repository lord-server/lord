...
--
lp_api.router     = {}
lp_api.router.cmd = core.settings:get("lp_api.router.cmd") or false

local SYS_NAME    = core.settings:get("lp_api.system_name") or "minetest"
	...
	if data.type == "cmd" and lp_api.router.cmd then
		...
		if data.command == "grant" then
			if data.args[2] then
				...
				local add_privs = core.string_to_privs(sp)
				local privs     = core.get_player_privs(data.args[1])
				for p, _ in pairs(add_privs) do
					privs[p] = true
				end
				core.set_player_privs(data.args[1], privs)
		...
		if data.command == "privs" then
			local privs = core.privs_to_string(core.get_player_privs(data.args[1]), ' ')
			lp_api.publisher.pub_msg(SYS_NAME, "system", "Privileges of {" .. data.args[1] .. "}:", privs)
		end

		if data.command == "status" then
			lp_api.publisher.pub_msg(SYS_NAME, "system", "Status server.", core.get_server_status())
		end
	end
end
