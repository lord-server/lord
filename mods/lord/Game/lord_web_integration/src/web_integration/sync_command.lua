...
local function create_player_on_web(player_name, notify_player_name)
	core.chat_send_player(notify_player_name, player_name .. ": send `::create()`")
	WebPlayer.create(
		core.get_player_by_name(player_name),
		core.get_auth_handler().get_auth(player_name).last_login,
		nil,
		function(created_player)
			core.chat_send_player(notify_player_name, player_name .. ":web_id: " .. created_player.id)
		end,
		function(linked_player)
			core.chat_send_player(notify_player_name, player_name .. ":linked web_id: " .. linked_player.id)
		end,
		function(result)
			core.chat_send_player(notify_player_name, player_name .. ":failed: " .. dump(result))
		end
	)
end
...
local function create_clan_on_web(clan, notify_player_name)
	core.chat_send_player(notify_player_name, "Clan " .. clan.name .. ": send `::create()`")

	WebClan.create(
		clan,
		function(created_clan)
			core.chat_send_player(notify_player_name, clan.name .. ":web_id: " .. created_clan.id)
		end,
		function(linked_clan)
			core.chat_send_player(notify_player_name, clan.name .. ":linked web_id: " .. linked_clan.id)
		end,
		function(result)
			core.chat_send_player(notify_player_name, clan.name .. ":failed: " .. dump(result))
		end
	)
end
...
local function sync_players(players_names, executor_name, iteration)
	for _, player_name in pairs(players_names) do
		...
		if not player_web_id then
			core.chat_send_player(executor_name, "Web Sync Player: " .. player_name)
			iteration = iteration + 1
			core.after(iteration, function()
...
local function sync_clans_handler(executor_name)
	core.chat_send_player(executor_name, "Web Sync Clans Start")
	local iteration = 0
	for _, clan in pairs(clans.list()) do
		core.chat_send_player(executor_name, "Web Sync Clan: " .. clan.name)
		iteration = sync_players(clan.players, executor_name, iteration)
		iteration = sync_clan(clan, executor_name, iteration)
	end

	core.chat_send_player(executor_name, "Web Sync Clans Finish")
end
...
return {
	...
	register = function(storage, web_player, web_clan)
		...
		core.register_chatcommand('web_sync_clans', {
			privs = { server = true },
