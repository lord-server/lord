...
-- Аргументы:
-- - chest_name - название ноды целевого сундука;
-- - possible_items - массив c массивами, описывающими лут в сундуках и его возможное кол-во, следующего вида:
--   {
...
local function register_chest_spawner(chest_name, possible_items)
	...
	local function get_items_available_from_possible_items(potential_items)
		...
		for _, item_table in ipairs(potential_items) do
			if #item_table == 1 then
				table.insert(items_available, item_table[1])
			elseif #item_table == 2 then
				table.insert(items_available, item_table[1] .. " " .. item_table[2])
			elseif #item_table == 3 then
				table.insert(items_available, item_table[1] .. " " .. math.random(item_table[2], item_table[3]))
			end
		end
