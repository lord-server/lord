...
--- @field speed_walk?      number  multiplier to default walk speed value (default: `1`). Note: The actual walk speed is the product of `speed` and `speed_walk`
--- @field speed_climb?     number  multiplier to default climb speed value (default: `1`). Note: The actual climb speed is the product of `speed` and `speed_climb`
--- @field speed_crouch?    number  multiplier to default crouch speed value (default: `1`). Note: The actual crouch speed is the product of `speed` and `speed_crouch`
--- @field speed_fast?      number  multiplier to default fast (sprint) speed value (default: `1`). Note: The actual fast speed is the product of `speed` and `speed_fast`
...
--- @field liquid_fluidity? number  multiplier to liquid movement resistance value (for nodes with `liquid_move_physics`); the higher this value, the lower the resistance to movement. At math.huge, the resistance is zero and you can move through any liquid like air. (default: `1`). Warning: Values below 1 are currently unsupported.
--- @field liquid_fluidity_smooth? number  multiplier to default maximum liquid resistance value (for nodes with `liquid_move_physics`); controls deceleration when entering node at high speed. At higher values you come to a halt more quickly (default: `1`)
--- @field liquid_sink?     number  multiplier to default liquid sinking speed value; (for nodes with `liquid_move_physics`), (default: `1`)
--- @field acceleration_default? number  multiplier to default acceleration value (default: `1`). Note: The actual acceleration is the product of `speed` and `acceleration_default`
--- @field acceleration_air?     number  multiplier to default air acceleration value (default: `1`). Note: The actual air acceleration is the product of `speed` and `acceleration_air`
--- @field acceleration_fast?    number  multiplier to default fast (sprint) acceleration (default: `1`). Note: The actual fast acceleration is the product of `speed` and `acceleration_fast`
--- @field sneak?           boolean whether player can sneak (default: `true`)
--- @field sneak_glitch?    boolean whether player can use the new move code replications of the old sneak side-effects: sneak ladders and 2 node sneak jump (default: `false`)
--- @field new_move?        boolean use new move/sneak code. When `false` the exact old code is used for the specific old sneak behavior (default: `true`)
